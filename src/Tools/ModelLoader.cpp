...
void ModelLoader::LoadSelectedModel() {
  if (WindowSystem::s_modelPath.has_value()) {
    ...
    try {
      ...
      Log::LogInfo("Model loaded: " +
                   WindowSystem::s_modelPath.value().string());
  ...
  if (m_waitingForUserInput == TEXTURE_SELECTION) {
    ...
    if (selectedTexturePath.has_value()) {
      if (selectedTexturePath.value() != "") {
        m_texture = loadTexture(selectedTexturePath.value().c_str(), GL_LINEAR);
        m_model->OverwriteTexture(m_texture);
  ...
  while (dir != end) {
    if (dir->is_regular_file()) {
      ...
      if (extension == ".png" || extension == ".jpg" || extension == ".jpeg" ||
        ...
        std::cout << "Texture found: " << dir->path().string() << std::endl;
